...
def get_projection_matrix_gaussian(znear, zfar, fovX, fovY, device):
    tanHalfFovY = math.tan((fovY / 2))  # cy / fy
    tanHalfFovX = math.tan((fovX / 2))  # cx / fx

    top = tanHalfFovY * znear   # znear的视锥
    bottom = -top
...
def get_projection_matrix_gaussian_lgm(znear, zfar, fovX, fovY, device):
...
def get_cam_info_gaussian(c2w, fovx, fovy, znear, zfar, device):
    # c2w = convert_pose(c2w)
    world_view_transform = torch.inverse(c2w.float())

    # world_view_transform = world_view_transform.transpose(0, 1).cuda().float()
    world_view_transform = world_view_transform.transpose(0, 1).to(device).float()
    projection_matrix = (
        get_projection_matrix_gaussian(znear=znear, zfar=zfar, fovX=fovx, fovY=fovy, device=device)
        .transpose(0, 1)
        .to(device)
    ).float()
    #     .cuda()
