...
class InpaintingDataset(torch.utils.data.Dataset):
    def __init__(self, test_size=None, use_gradient=False, img_file_list=None, egopart_img_file_list=None, key_mask_file_list=None, sample_token_list=None, sensor_list=None):
        super(InpaintingDataset, self).__init__()
        self.test_size = test_size
        self.use_gradient = use_gradient
        
        self.img_file_list = img_file_list
        self.egopart_img_file_list = egopart_img_file_list
        self.key_mask_file_list = key_mask_file_list
        self.sample_token_list = sample_token_list
        self.sensor_list = sensor_list

        print('Image num:', len(self.img_file_list))
        print('Mask num:', len(self.img_file_list))
    ...
    def __getitem__(self, index):
        ...
        egopart_img_file = self.egopart_img_file_list[index]
        key_mask_file = self.key_mask_file_list[index]
        sample_token = self.sample_token_list[index]
        sensor = self.sensor_list[index]
        
        key_mask = cv2.imread(key_mask_file)
        # ! HACK DEBUG
        key_mask = cv2.resize(key_mask, (800, 450), interpolation=cv2.INTER_NEAREST)
        egopart_mask = key_mask[..., 2] == 250
        mask = (key_mask[..., 2] == 125).astype(np.uint8) * 255
        
        img = cv2.imread(img_file)
        if os.path.exists(egopart_img_file):
            egopart_img = cv2.imread(egopart_img_file)
            # ! HACK DEBUG
            egopart_img = cv2.resize(egopart_img, (img.shape[1], img.shape[0]))
            img[egopart_mask] = egopart_img[egopart_mask]
        else:
            egopart_img = np.zeros_like(img)

        batch = dict()
        batch['idx'] = index
        batch['key_mask'] = key_mask
        batch['sample_token'] = sample_token
        batch['sensor'] = sensor
        batch['img_hw_raw'] = [img.shape[0], img.shape[1]]
