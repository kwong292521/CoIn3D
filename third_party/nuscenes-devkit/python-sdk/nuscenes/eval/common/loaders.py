...
from nuscenes.eval.detection.utils import category_to_detection_name, category_to_detection_name_lyft_gen3cls, category_to_detection_name_waymo_gen3cls, category_to_detection_name_nuscenes_gen3cls, category_to_detection_name_lyft
from nuscenes.eval.tracking.data_classes import TrackingBox
from nuscenes.utils.data_classes import Box
from nuscenes.utils.geometry_utils import points_in_box
from nuscenes.utils.splits import create_splits_scenes, get_scenes_of_custom_split, create_splits_scenes_lyft, create_splits_scenes_waymo
...
def load_gt(nusc: NuScenes, eval_split: str, box_cls, verbose: bool = False, cde_setting: dict = None) -> EvalBoxes:
    """
    ...
    if cde_setting is None:
        splits = create_splits_scenes()
    else:
        if cde_setting['dst_dataset'] == 'nuscenes':
            splits = create_splits_scenes()
        elif cde_setting['dst_dataset'] == 'lyft':
            splits = create_splits_scenes_lyft()
        elif cde_setting['dst_dataset'] == 'waymo':
            splits = create_splits_scenes_waymo()
        ...
        for sample_annotation_token in sample_annotation_tokens:
            ...
            if box_cls == DetectionBox:
                ...
                if cde_setting is None:
                    ...
                    if cde_setting['dst_dataset'] == 'nuscenes':
                        if cde_setting['dst_eval_cls'] == 'general_3cls':
                            # detection_name = category_to_detection_name(sample_annotation['category_name'])
                            detection_name = category_to_detection_name_nuscenes_gen3cls(sample_annotation['category_name'])
                        else:
                            detection_name = category_to_detection_name(sample_annotation['category_name'])
                    elif cde_setting['dst_dataset'] == 'lyft':
                        if cde_setting['dst_eval_cls'] == 'general_3cls':
                            detection_name = category_to_detection_name_lyft_gen3cls(sample_annotation['category_name'])
                        else:
                            # assert False
                            detection_name = category_to_detection_name_lyft(sample_annotation['category_name'])
                            
                    elif cde_setting['dst_dataset'] == 'waymo':
                        if cde_setting['dst_eval_cls'] == 'general_3cls':
                            detection_name = category_to_detection_name_waymo_gen3cls(sample_annotation['category_name'])
                        else:
                            assert False
