...
def create_splits_scenes_lyft(verbose=False):
    cur_dir = os.path.dirname(os.path.abspath(__file__))
    
    with open(os.path.join(cur_dir, 'splits/lyft/train.txt'), 'r') as f:
        train_scenes = f.read().splitlines()
    with open(os.path.join(cur_dir, 'splits/lyft/val.txt'), 'r') as f:  
        val_scenes = f.read().splitlines()
    with open(os.path.join(cur_dir, 'splits/lyft/test.txt'), 'r') as f:
...
def create_splits_scenes_waymo(verbose=False):
    cur_dir = os.path.dirname(os.path.abspath(__file__))
    
    with open(os.path.join(cur_dir, 'splits/waymo/train.txt'), 'r') as f:
        _scenes = f.read().splitlines()
        _scenes = [item[:4] for item in _scenes]
        uni_scenes = np.unique(_scenes)
        train_scenes = [f'scene-{item}' for item in uni_scenes]
    with open(os.path.join(cur_dir, 'splits/waymo/val.txt'), 'r') as f:
        _scenes = f.read().splitlines()
        _scenes = [item[:4] for item in _scenes]
        uni_scenes = np.unique(_scenes)
        val_scenes = [f'scene-{item}' for item in uni_scenes]
