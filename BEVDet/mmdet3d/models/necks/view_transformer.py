...
class LSSViewTransformer(BaseModule):
    ...
    def __init__(
        ...
        self.sid = sid
        # D x H x W x 3 (u, v, d) for each pixel, d is predefined depth anchor, bevdepth will use estimated depth distribution to proj 2d feat to bev
    ...
    def view_transform(self, input, depth, tran_feat):
        for shape_id in range(3):
            assert depth.shape[shape_id+1] == self.frustum.shape[shape_id], f'depth shape {depth.shape} != frustum shape {self.frustum.shape}'
        if self.accelerate:
...
class DepthNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.block_cam_aware = block_cam_aware
    ...
    def forward(self, x, mlp_input, stereo_metas=None):
        if self.block_cam_aware:
            mlp_input = torch.ones_like(mlp_input)
        mlp_input = self.bn(mlp_input.reshape(-1, mlp_input.shape[-1]))     # (n_cam, 27)
        x = self.reduce_conv(x)                                             # (512, 16x) -> (512, 16x)
        context_se = self.context_mlp(mlp_input)[..., None, None]           # (n_cam, 512, 1, 1)
        context = self.context_se(x, context_se)        
        context = self.context_conv(context)            # NOTE: channel 512->80
        depth_se = self.depth_mlp(mlp_input)[..., None, None]       # (n_cam, 512, 1, 1)
        ...
        if self.with_cp:
            ...
            depth = self.depth_conv(depth)          # NOTE: channel 512->118
...
class LSSViewTransformerBEVDepth(LSSViewTransformer):
    ...
    def get_downsampled_gt_depth(self, gt_depths):
        ...
        gt_depths = gt_depths.view(B * N, H // self.downsample,
                                   ...
                                   self.downsample, 1)
        gt_depths = gt_depths.permute(0, 1, 3, 5, 2, 4).contiguous()    # (b*n, h/ds, h_ds, w/ds, w_ds, 1) -> (b*n, h/ds, w/ds, 1, h_ds, w_ds)
        gt_depths = gt_depths.view(-1, self.downsample * self.downsample)   # (b*n * h/ds * w/ds, h_ds*w_ds)
    ...
    def forward(self, input, stereo_metas=None):
        ...
        B, N, C, H, W = x.shape
        x = x.view(B * N, C, H, W)
        x = self.depth_net(x, mlp_input, stereo_metas)      # [198, 16, 44] 198=118+80
        depth_digit = x[:, :self.D, ...]                    
        tran_feat = x[:, self.D:self.D + self.out_channels, ...]
        depth = depth_digit.softmax(dim=1)
        bev_feat, depth = self.view_transform(input, depth, tran_feat)  # bev_feat:# [b, 80, 128, 128] 
        return bev_feat, depth
