...
if __name__ == '__main__':
    ...
    # dataset = 'lyft'
    # dataset = 'waymo'
    img_dir = os.path.join('./test_data', dataset, 'img')
    mask_dir = os.path.join('./test_data', dataset, 'mask')
    save_path = os.path.join('./test_data', dataset, 'output')
    
    args = argparse.ArgumentParser(description='PyTorch Template')
    args.add_argument('--config', type=str, help='config file path', 
                      default="configs/config_zitspp_finetune.yml")
    args.add_argument('--exp_name', type=str, help='method name', 
                      default='model_512')     # <your model name>
    args.add_argument('--dynamic_size', action='store_true', help='Whether to finetune in dynamic size?')
    args.add_argument('--use_ema', help='Whether to use ema?',
                    #   action='store_true',
                      default=True)
    args.add_argument('--ckpt_resume', type=str, help='PL path to restore', 
                      default='ckpts/model_512/models/last.ckpt')
    args.add_argument('--wf_ckpt', type=str, help='Line detector weights', 
                      default='ckpts/best_lsm_hawp.pth')
    args.add_argument('--save_path', type=str, help='path to save', 
                      default=save_path)    # TODO:
    args.add_argument('--img_dir', type=str, help='Test image path', 
                      default=img_dir)     # TODO:
    args.add_argument('--mask_dir', type=str, help='Mask path', 
                      default=mask_dir)     # TODO:
    ...
    args.add_argument('--save_image_only', help='Only save image', 
                      action='store_true',
                      default=True
                      )
    args.add_argument('--obj_removal', help='obj_removal', 
                    #   action='store_true',
                      default=True)
