...
class SPNet:
    def __init__(self, cf, rank):
        ...
        self.rank = rank
        self.network = (
            V2Net(cf.dims, cf.depths, cf.dp_rate, cf.norm_type).cuda().train()
        )
        if rank == 0:
        ...
        warmup_iters = self.iteration_num * cf.warmup_epochs
        total_iters = self.iteration_num * cf.epochs
        lr_lambda = lambda x: (
            x / warmup_iters
            if x < warmup_iters
            else 0.5
            * (
                1
                + math.cos(math.pi * (x - warmup_iters) / (total_iters - warmup_iters))
            )
        )
        ...
        for epoch in range(self.start_epoch + 1, cf.epochs + 1):
            ...
            if self.rank == 0:
                ...
                if epoch % cf.checkpoint_epoch == 0 or epoch == cf.epochs:
                    save_file = model_dir / f"{cf.model_type}_epoch_{epoch}.pth"
                    torch.save(
                        {
                            "epoch": epoch,
