...
from .prior_utils import PriorMapGenerator, get_prior_modulated_module
...
class BEVDet(CenterPoint):
    ...
    def __init__(self,
        ...
        if prior_cfg is not None:
            self.prior_generator = PriorMapGenerator(prior_cfg, num_adj+1)
            PriorModulatedLayer = get_prior_modulated_module(prior_cfg['feat_modulate']['type'])

            self.prior_modulate_layer_backbone_16x = PriorModulatedLayer(1024, prior_cfg, 'after_backbone')
            self.prior_modulate_layer_backbone_32x = PriorModulatedLayer(2048, prior_cfg, 'after_backbone')
            self.prior_modulate_layer_neck = PriorModulatedLayer(512, prior_cfg, 'after_neck')
        
            
    def prior_adapt(self):
        if self.prior_cfg is None: 
            return
        
        if self.prior_cfg['feat2cat']['img_input']:
            conv1_weight = self.img_backbone.conv1.weight.clone().detach()
            self.img_backbone.conv1 = nn.Conv2d(3+self.prior_cfg['prior_channel'], 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
            with torch.no_grad():
                self.img_backbone.conv1.weight[:, :3, :, :] = conv1_weight
    ...
    def image_encoder(self, img, stereo=False):
        ...
        x = self.img_backbone(imgs)     # NOTE: 两个尺度的特征 16/32倍下采样 channel:1024/2048
        stereo_feat = None
        if stereo:
            stereo_feat = x[0]
            x = x[1:]
        if self.with_img_neck:
            x = self.img_neck(x)        # NOTE: 得到16倍下采样后的特征 channel:512
    ...
    def image_encoder_with_priormap(self, img, prior_map, stereo=False):
        imgs = img
        B, N, C, imH, imW = imgs.shape
        imgs = imgs.view(B * N, C, imH, imW)
        if self.grid_mask is not None:
        ...
        if self.prior_cfg:
            prior_map_16x = self.prior_generator.downsample_maps([prior_map], 16)[0]
            prior_map_32x = self.prior_generator.downsample_maps([prior_map], 32)[0]
            prior_map_16x = prior_map_16x.view(B*N, prior_map_16x.shape[2], prior_map_16x.shape[3], prior_map_16x.shape[4])
            prior_map_32x = prior_map_32x.view(B*N, prior_map_32x.shape[2], prior_map_32x.shape[3], prior_map_32x.shape[4])
        
        # HACK implementation
        if self.prior_cfg:
            x_16x, x_32x = x
            x_16x = self.prior_modulate_layer_backbone_16x(x_16x, prior_map_16x)
            x_32x = self.prior_modulate_layer_backbone_32x(x_32x, prior_map_32x)
            x = (x_16x, x_32x)
        ...
        if self.with_img_neck:
            ...
            if self.prior_cfg:
                x = self.prior_modulate_layer_neck(x, prior_map_16x)
...
class BEVDet4D(BEVDet):
    ...
    def __init__(self,
                 ...
                 with_prev=True,
                 **kwargs):
        super(BEVDet4D, self).__init__(num_adj=num_adj, **kwargs)
    ...
    def prepare_bev_feat(self, img, rot, tran, intrin, post_rot, post_tran,
                         bda, mlp_input, prior_map=None):
        if prior_map is None:
            x, _ = self.image_encoder(img)
        else:
            x, _ = self.image_encoder_with_priormap(img, prior_map)
    ...
    def extract_img_feat(self,
        ...
        bda, _ = self.prepare_inputs(img)   # img(tuple, each is (bs, n_cam/n_adj, ...)); imgs,...(list, len=n_adj+1, each is (bs, n_cam, ...))
        
        # if self.prior_generator is not None:
        if self.prior_cfg:
            prior_maps = self.prior_generator(img, test_mode=(not self.training))
            # prior_maps_16x = self.prior_generator.downsample_maps(prior_maps, 16)
            # PriorMapGenerator._vis_prior_maps(prior_maps[0], imgs[0])
            if self.prior_cfg['feat2cat']['img_input']:
                for i in range(len(imgs)):
                    imgs[i] = torch.cat([imgs[i], prior_maps[i]], dim=2)
            
        """Extract features of images."""
        bev_feat_list = []
        depth_list = []
        key_frame = True  # back propagation for key frame only
        for frame_idx, (img, sensor2keyego, ego2global, intrin, post_rot, post_tran) in enumerate(zip(
                imgs, sensor2keyegos, ego2globals, intrins, post_rots, post_trans)):
            if key_frame or self.with_prev:
                ...
                if self.prior_cfg is None:
                    ...
                    inputs_curr = (img, sensor2keyego, ego2global, intrin, post_rot,
                                post_tran, bda, mlp_input, prior_maps[frame_idx])
                if key_frame:
                    bev_feat, depth = self.prepare_bev_feat(*inputs_curr)
                else:
                    with torch.no_grad():
                ...
                depth = None
            bev_feat_list.append(bev_feat)      # [b, 80, 128, 128]
            depth_list.append(depth)            # [n_cam, 118, 16, 44]
            key_frame = False
        if pred_prev:
            ...
            for adj_id in range(1, self.num_frame):
                bev_feat_list[adj_id] = \
                    self.shift_feature(bev_feat_list[adj_id],
                                       ...
                                       bda)
        bev_feat = torch.cat(bev_feat_list, dim=1)
        x = self.bev_encoder(bev_feat)      # [b, 256, 128, 128]
