...
_base_ = ['../../_base_/datasets/nusc_format_multi_datasets/lyft-3d.py', '../../_base_/default_runtime.py']
# Global
...
train_pipeline = [
    dict(
        ...
        aug_cam2ego=[0.2, 0.2, ('range', 1.5, 2.2), 0, 0, 20],       # [tx, ty, tz, rx, ry, rz] # ! tz-> nuscenes+lyft:('range', 1.5, 1.7), nuscenes+waymo:('range', 2.0, 2.2)
        aug_K=['ratio', 0.7, 1.4],   
        aug_K_on_raw=['ratio', 0.94, 1.4],
        # depth supervision
        use_lidar_depth=True,
        fg_depth_only='car',
        # lyft ego-part inpaint
        inpaint_ego_occ_train=True,
        # sync parameters
...
test_data_config = dict(
    ...
    ann_file=data_root + 'lyft_infos_val_bevdet_format_general_3cls_sync+raw.pkl')
    # ann_file=data_root + 'lyft_infos_val_bevdet_format_general_3cls.pkl')


# ! Plain dataset(nocbgs) 
data = dict(
    samples_per_gpu=8,         
    workers_per_gpu=4,
    train=dict(
        data_root=data_root,
        # ann_file=data_root + 'lyft_infos_train_bevdet_format_general_3cls.pkl',
        ann_file=data_root + 'lyft_infos_train_bevdet_format_general_3cls_sync+raw.pkl',
