...
class RandomDepthScaling(torch.nn.Module):
    def __init__(self, factor):
        super(RandomDepthScaling, self).__init__()
        self.factor = factor
    ...
    def __init__(self, size):
        self._rgbgt_transform = trans.Compose(
            [
                ...
                RandomResizedCropRGBD(size, (0.64, 1.0), antialias=True),
                trans.RandomHorizontalFlip(0.5),
            ]
        )
        self._rgb_transform = trans.ColorJitter(0.2, 0.2, 0.2)
        self._gt_transform = RandomDepthScaling(0.2)
        ...
        self.pro_list = [0, 0.5, 1.0]  # probability range of X for depth completion
